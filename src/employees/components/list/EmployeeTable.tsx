...
import { TableSkeleton } from '@/src/shared/components/loaders/TableSkeleton';
import NoRecords from '@/src/shared/components/utils/NoRecords';
        ...
        <tbody className="divide-y divide-gray-100">
          {employees?.map((employee) => (
            ...
            >
              ...
              {/* Role */}
              <td className="px-3 py-2.5">
                <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-medium ${roleColor(String(employee.role))}`}>
                  <Circle className={`w-1.5 h-1.5 fill-current ${roleDot(String(employee.role))}`} />
                  {roleLabel(String(employee.role))}
                </div>
              </td>
